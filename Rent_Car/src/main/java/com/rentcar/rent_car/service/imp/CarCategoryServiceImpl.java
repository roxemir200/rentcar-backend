...
public class CarCategoryServiceImpl implements CarCategoryService {
    ...
    public MessageResponse deleteCategory(Long id) {
        if (!categoryRepository.existsById(id)) {
            return MessageResponse.error("Catégorie non trouvée");
        }

        // Vérifier si des voitures utilisent cette catégorie
        long carCount = carRepository.findByCategoryId(id).size();
        if (carCount > 0) {
            return MessageResponse.error("Impossible de supprimer : " + carCount + " voiture(s) utilisent cette catégorie");
        }
